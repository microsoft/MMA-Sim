...
from mmasim.nv import shape_to_mnk, torch_dtype, min_exponent
...
class DotAdd:
    ...
    def __call__(self, a: list[float], b: list[float], c: float = 0.0) -> float:
        ...
        B_T = torch.zeros([n, k], dtype=torch_dtype[self.b_type])
        ...
        B = B_T.T.cuda()
        if self.is_wgmma:
            D = torch.zeros([m, n], dtype=torch_dtype[self.d_type])
            D[0, 0] = c
            D = D.cuda()
            self.mma(D.data_ptr(), A.data_ptr(), B.data_ptr())
        else:
            ...
            C = C.cuda()
            D = D.cuda()
            self.mma(D.data_ptr(), A.data_ptr(), B.data_ptr(), C.data_ptr())
        return D[0, 0].item()
