...
def random_test(
    ...
    for _ in range(trials):
        ...
        B = B_T.T
        C = (
            torch.randint(0, 256, [m * n * c_type.itemsize], dtype=torch.uint8)
            .view(c_type)
            .view(m, n)
        )
        if has_block_scale:
            ...
            scale_A = (
                torch.randint(
                    0,
                    256,
                    [m * k // block_size * s_type.itemsize],
                    dtype=torch.uint8,
                )
                .view(s_type)
                .view(m, k // block_size)
            ...
            scale_B_T = (
                torch.randint(
                    0,
                    256,
                    [n * k // block_size * s_type.itemsize],
                    dtype=torch.uint8,
                )
                .view(s_type)
                .view(n, k // block_size)
        ...
        if is_different.any():
            ...
            print(f"    D_real[i, j] = {float(D_real[i,j].item()).hex()}, {D_real[i, j].view(storage_type[d_type.itemsize])}")
            print(f"    D_sim[i, j] = {float(D_sim[i,j].item()).hex()}, {D_sim[i, j].view(storage_type[d_type.itemsize])}")
